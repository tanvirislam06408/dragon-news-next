...
const NewsCard = ({ newsD, detailsCard }) => {
        ...
        <div className='mt-6 border p-3 border-gray-300'>
            <div className='flex items-center justify-between bg-[#F3F3F3] p-2.5'>
                <div className="flex items-center gap-3">
                    <Image className='rounded-full' src={author.img} width={40} height={40} alt='card avatar' />
                    <div className="">
            ...
            {detailsCard === 'allCard' ? <p className=' mt-3 text-[16px] text-gray-500'>{details.slice(1, 200)}...</p> : <p className=' mt-3 text-[16px] text-gray-500'>{details}</p>}
            {detailsCard === 'allCard' && <Link href={`/news/${_id}`} className='btn text-orange-500 btn-outline'>Read More</Link>}
        </div>
    );
};
